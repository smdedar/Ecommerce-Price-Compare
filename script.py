...
def pickaboo_search(product):
    ...
    product_names = soup.find_all(
        "strong", attrs={"class": "product name product-item-name"}
    )
    ...
    df["icon"] = (
        "https://b2b-pickaboocdn.s3-ap-southeast-1.amazonaws.com/media/logo/stores/1/pickaboo-logo.png"
    )
...
def priyoshop_search(product):
    ...
    df = pd.concat([df_img, df_name, df_price, df_link], axis=1)
    df["icon"] = (
        "https://s3-ap-southeast-1.amazonaws.com/priyoshop/content/Images/thumbs/0072813.png"
    )
...
def techland_search(product):
    ...
    df = pd.concat([df_img, df_name, df_price, df_link], axis=1)
    df["icon"] = (
        "https://www.techlandbd.com/image/cache/catalog/techland/logo/techland-logo-white-300-300x48.png"
    )
...
def computersource_search(product):
    ...
    df = pd.concat([df_img, df_name, df_price, df_link], axis=1)
    df["icon"] = (
        "https://www.computersourcebd.com/asset/view/new/assets/img/logo/logo2.png"
    )
